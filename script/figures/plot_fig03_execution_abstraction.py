"""Paper figure 3: execution abstraction for EFTQC architectures.

Both architectures use probabilistic resource preparation followed by
teleportation-based consumption. At minimal scale -- one logical qubit and two
factories -- this contrasts STAR's TMR preparation plus RUS teleportation (top
row) against T-cultivation's check stage, escape stage, and teleportation
(bottom row).
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Paper figure 3: two-row execution timeline, STAR (top) and "
            "T-cultivation (bottom), each with 1 logical qubit and 2 factories."
        )
    )
    parser.add_argument(
        "--output-pdf",
        default="output/figures/fig03_execution_abstraction_star_vs_t_cultivation.pdf",
        help="Output path for the figure 3 PDF",
    )
