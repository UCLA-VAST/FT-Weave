"""Paper figures 5 and 6: effect of compilation-strategy optimization.

Runs identical-angle ``Rz`` gates on 25 logical qubits with 25 factories and a
single AOD, under three strategies -- an unoptimized baseline, the coordinated
assignment and movement optimizations, and the same plus resource
rematerialization -- and writes:

* figure 5, the three stacked execution timelines;
* figure 6, the per-teleportation-round movement schedules on a distance-3
  surface-code toy layout, one directory per strategy.

Figure 6's three columns in the paper are the rounds highlighted in figure 5:
round 1 for the baseline and optimized strategies, round 5 for the
rematerialization strategy (see ``_ROW_RUS_MOVE_SHADE_ROUNDS``).
"""
...
# These are the rounds figure 6 shows as its three movement-schedule columns.
...
# Directory slug per strategy row for the figure 6 movement schedules. Kept
# separate from the in-figure row labels so renaming a legend entry does not
# rename output directories.
_ROW_OUTPUT_SLUGS = [
    "baseline",
    "optimized",
    "optimized_with_rematerialization",
]
...
def _run_star_log(
    *, optimize_strategy: bool, consider_skip_rus: int = 0, rng: np.random.Generator
    ...
    """Compile one strategy's execution log for the figure 5 / 6 workload."""
...
def _plot_rus_rounds_for_rows(
    ...
    *,
    ...
    for slug, (label, log, _nf, _nq, logic_locs, magic_locs, _placement) in zip(
        _ROW_OUTPUT_SLUGS, row_plots
    ):
        base_path = os.path.join(rus_output_dir, slug)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Paper figures 5 and 6: three stacked STAR execution timelines for "
            "identical-angle Rz gates on 25 logical qubits with 25 factories "
            "and one AOD (unoptimized baseline / optimized / optimized with "
            "resource rematerialization), plus the per-round movement schedules."
        )
    )
    parser.add_argument(
        "--output-pdf",
        default=("output/figures/fig05_execution_timeline_compilation_strategies.pdf"),
        help="Output path for the figure 5 timeline PDF",
    )
    parser.add_argument(
        "--figure-width",
        type=float,
    ...
    parser.add_argument(
        "--rus-output-dir",
        default="output/figures/fig06_movement_schedules",
        help="Directory for the figure 6 per-round movement-schedule PDFs",
    )
    parser.add_argument(
        "--rus-code-distance",
        type=int,
